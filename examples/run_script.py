...
def main():
    ...
    fig1, _ =  occupancy_plot(pos = traj["pos"], track_length = traj["meta"]["track_length"], n_bins = 50)
    ...
    peak_rates = np.random.uniform(10, 20, n_cells)
    baseline_rate = 0.5
    ...
    spikes, spike_times, spike_pos, spike_counts = generate_place_cell_spikes(centers = centers, sigma_pf = sigmas, peak_rates = peak_rates, 
    ...
    empirical_rate_maps, bin_centers = compute_empirical_rate_maps(n_bins = 100, track_length = traj["meta"]["track_length"],
    pos = traj["pos"], dt = 0.005, spike_positions = spike_pos) 
    
    theoretical_rates, x = compute_theoretical_rate_maps(n_bins = 100, track_length=traj["meta"]["track_length"], centers = centers, sigma_pf = sigmas,
    peak_rates = peak_rates, baseline_rate = baseline_rate)

    # generate spikes for noise cells
    noise_spikes = generate_noise_cell_spikes(n_noise = n_noise, min_rate = min_noise_rate, max_rate = max_noise_rate,  
        duration_s = traj["meta"]["duration_s"], dt = 0.005)

    # save dataset (trajectory only for now) and additional arrays
    save_dataset({"time": traj["time"], "pos": traj["pos"], "vel": traj["velocity"], "meta": traj["meta"], 
    "spikes": traj["place_cell_spikes"], "noise_spikes": traj["noise_cell_spikes"]}, args.out)
    print("Saved dataset to", args.out)

    # create figures with spike raster and empirical vs theoretical rate maps of place cells 
    fig3, _ = plot_spike_raster(spike_times = spike_times, duration = traj["meta"]["duration_s"], centers = centers)
